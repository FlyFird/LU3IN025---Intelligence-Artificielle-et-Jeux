...
def strat_stochastique(p, x_init, y_init, nb_lignes, nb_cols, pos_restaurants, nb_restos):
    """
        Cette fonction calcule et retourne le chemin que le joueur p va parcourir pour aller a un restaurant tire aleatoirement
    ...
    """
    path = []
    choix_resto = pos_restaurants[random.randint(0, nb_restos-1)]
    pos_player = (x_init,y_init[p])

    print("Le client :", p, "avec la stratégie stochastique et il commence en :", pos_player)
    print("Il décide d'aller dnas le restaurant :", choix_resto)
