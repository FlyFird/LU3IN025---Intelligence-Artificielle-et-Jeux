...
class Robot_player(Robot):
    ...
    def follow_enemy(sensors) :
        ...
        if enemy_around(sensors) :
            rotation = (-1) * sensors[7]["distance_to_enemy"] + 1 * sensors[0]["distance_to_enemy"] + \
            1 * sensors[7]["distance_to_wall"] + (-1) * sensors[0]["distance_to_wall"]
            return 1, rotation
        for i in range(len(sensors)) :
            if sensors[i]["distance_to_wall"] < 0.05 : #si proche d'un mur, adopte le comportement qui évite les murs
                return self.hate_wall(sensors)
        else:
    ...
    def spirale(self):
        ...
        spiral= (self.memory % 10000) / 10000
    ...
    def strat_adapt_wanderer(self, sensors):
        ...
        def hate_ally(sensors):
            ...
            # renvoie les valeurs de translation et de rotation
            ...
            translation = (0.2)*sensors[0]["distance_to_ally"]
        ...
        def hate_wall(sensors):
            ...
            translation = (0.2)*sensors[0]["distance_to_wall"]
            rotation =  front + mid + counter_perp
        ...
        def strat_par_defaut(sensors):
            ...
            if (((sensors[0]["distance_to_wall"] <= 0.8) and (sensors[1]["distance_to_wall"] <= 0.8)) or ((sensors[0]["distance_to_wall"] <= 0.8) and (sensors[7]["distance_to_wall"] <= 0.8))):
                translation, rotation, _ = self.hate_all(sensors)
            # Si on detecte un robot, on a un comportement qui suit le robot
            elif (((sensors[0]["distance_to_ally"] <= 0.8) and (sensors[1]["distance_to_ally"] <= 0.8)) or ((sensors[0]["distance_to_ally"] <= 0.8) and (sensors[7]["distance_to_ally"] <= 0.8))):
                translation, rotation, _ = self.hate_all(sensors)
        ...
        if (nb_step <= 100):
            ...
            if ((nb_step > 0) and (nb_step % 100 == 0)):
    ...
    def walker(self,sensors) :
        ...
        tmp1,tmp2,tmp3=self.hate_all(sensors)
        if tmp2==1.0:
            return self.spirale(sensors)
        return self.hate_all(sensors)
