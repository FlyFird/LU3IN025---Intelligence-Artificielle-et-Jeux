...
class Robot_player(Robot):
    ...
    def detecteur_wall(self,sensors) :
        ...
        if (sensors[0]["distance_to_wall"] < 0.7 or
            ...
            sensors[5]["distance_to_wall"] < 0.7):
            return True
        return False

    def ally_around(self,sensors, mode) :
    ...
    def tourne_right(self,sensors):
        ...
        if near<1.0 and front_right<1.0 and near_right<1.0 and right>0.7:
            return False
        if right>0.7:
            return True
        if front_right>0.5:
    ...
    def tourne_left(self,sensors):
        ...
        if near<1.0 and front_left<1.0 and near_left<1.0 and left>0.7:
            return False
        if left>0.7:
            return True
        if front_left>0.5:
    ...
    def evite_obstacle_front(self, sensors):
        ...
        if front < 0.15:
            ...
            print(diff)
            if right<0.3 and left<0.3 or sensors[4]["distance"]<0.3:
        ...
        if front<0.35:
            if front_right-front_left>0.15:
                print("Contourne par la droite")
                return 0.2, -0.7, False
            else:
                print("Contourne par la gauche")
                return 0.2, 0.7, False
        if front_right<0.3:
            print("FRONT gauche")
        ...
        if left<0.15:
            ...
            return 0.7,-0.1,False
        diff = (right+front_right) - (left+front_left)
        if sensors[4]["distance"] < 1.0 and sensors[0]["distance"] <1.0 and diff>=0.0:
            print("gauche")
    ...
    def strat_love_wall(self,sensors):
        ...
        if self.detecteur_wall(sensors):
            ...
            if (sensors[2]["distance"]<1.0 and sensors[6]["distance"]<1.0 or
                ...
                if sensors[2]["distance"]<sensors[6]["distance"]:
    ...
    def strat_adapt_wanderer(self, sensors):
        ...
        def strat_par_defaut(sensors):
            ...
            if ((sensors[0]["distance_to_wall"] <= 0.8) and (sensors[1]["distance_to_wall"] <= 0.8) and (sensors[7]["distance_to_wall"] <= 0.8)):
                translation, rotation, _ = hate_wall(sensors)
            # Si on detecte un robot, on a un comportement qui suit le robot
                ...
                translation, rotation, _ = hate_ally(sensors)
            # Sinon on fait un wanderer
            else:
                translation = (0.8)*sensors[0]["distance_to_wall"]
        ...
        def strat_rapide_grand_angle(sensors):
            ...
            # Si on avance
            if (translation >= 0):
                translation = translation + (0.3)*(min(sensors[0]["distance_to_wall"], sensors[0]["distance_to_ally"]))
            # Sinon on recule
            else:
                translation = translation - (0.3)*(min(sensors[0]["distance_to_wall"], sensors[0]["distance_to_ally"]))
            ...
            if (rotation >= 0):
                rotation = rotation + 1
            # Sinon on tourne vers la gauche
            else:
                rotation = rotation - 1
