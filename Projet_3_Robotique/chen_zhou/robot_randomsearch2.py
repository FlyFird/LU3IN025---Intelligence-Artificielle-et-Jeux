from robot import *
import math
import random

nb_robots = 0
debug = False

class Robot_player(Robot):

    team_name = "Optimizer"
    robot_id = -1
    iteration = 0

    param = []
    bestParam = [] #param
    evaluations = 500
    it_per_evaluation = 400
    trial = 0 #pos

    current_eval = 0 # évaluation courante pour un paramètre donné (0,1 ou 2)

    x_0 = 0
    y_0 = 0
    theta_0 = 0



    #ajout sinon je ne vois pas comment le faire
    bestScore = 0.0
    currentScore = 0.0
    paramScore = 0.0
    cumulative_score=0.0
    current_repeat = 0

    def __init__(self, x_0, y_0, theta_0, name="n/a", team="n/a", evaluations=0, it_per_evaluation=0):
        global nb_robots
        self.robot_id = nb_robots
        nb_robots += 1
        self.x_0 = x_0
        self.y_0 = y_0
        self.theta_0 = theta_0
        self.param = [random.randint(-1, 1) for i in range(8)]
        self.evaluations = evaluations
        self.it_per_evaluation = it_per_evaluation
        super().__init__(x_0, y_0, theta_0, name=name, team=team)

    def reset(self):
        self.theta = random.uniform(0, 2*math.pi)
        super().reset()

    def step(self, sensors, sensor_view=None, sensor_robot=None, sensor_team=None):

        # cet exemple montre comment générer au hasard, et évaluer, des stratégies comportementales
        # Remarques:
        # - l'évaluation est ici la somme des distances parcourues par pas de temps, mais on peut en imaginer d'autres
        # - la liste "param", définie ci-dessus, permet de stocker les paramètres de la fonction de contrôle
        # - la fonction de controle est une combinaison linéaire des senseurs, pondérés par les paramètres (c'est un "Perceptron")

        # toutes les X itérations: le robot est remis à sa position initiale de l'arène avec une orientation aléatoire
        if self.iteration % self.it_per_evaluation == 0 and self.randome:
                if self.iteration > 0:
                    self.cumulative_score += self.currentScore
                    self.current_repeat += 1
                    if self.current_repeat<3:
                         self.currentScore=0.0
                         self.iteration+=1
                         return 0,0,True
                    else:
                        if self.cumulative_score>=self.bestScore:
                            self.Score = self.cumulative_score
                            self.bestParam = self.param.copy()
                            self.bestEval = self.trial
                            self.currentScore=0.0
                    print ("translations =",self.log_sum_of_translation,"; rotations =",self.log_sum_of_rotation)
                    self.param = [random.randint(-1, 1) for i in range(8)]
                    self.trial = self.trial + 1
                    print ("Trying strategy no.",self.trial)
                    self.iteration = self.iteration + 1
                    return 0, 0, True # ask for reset

        # fonction de contrôle (qui dépend des entrées sensorielles, et des paramètres)
        translation = math.tanh ( self.param[0] + self.param[1] * sensors[sensor_front_left] + self.param[2] * sensors[sensor_front] + self.param[3] * sensors[sensor_front_right] )

        rotation = math.tanh ( self.param[4] + self.param[5] * sensors[sensor_front_left] + self.param[6] * sensors[sensor_front] + self.param[7] * sensors[sensor_front_right] )
        self.currentScore += translation * (1 - abs(rotation))
        if self.iteration %self.evaluations:
                self.iteration = 0
                self.randome=False
                self.param = self.bestParam[:]
                self.reset()
        if debug == True:
            if self.iteration % 100 == 0:
                print ("Robot",self.robot_id," (team "+str(self.team_name)+")","at step",self.iteration,":")
                print ("\tsensors (distance, max is 1.0)  =",sensors)
                print ("\ttype (0:empty, 1:wall, 2:robot) =",sensor_view)
                print ("\trobot's name (if relevant)      =",sensor_robot)
                print ("\trobot's team (if relevant)      =",sensor_team)

        self.iteration = self.iteration + 1

        return translation, rotation, False